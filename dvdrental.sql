...
SELECT full_name, 
       ...
       CASE 
         ...
         WHEN film_length > 60 
              AND film_length <= 120 THEN 'Between 1 - 2 hours' 
         WHEN film_length > 12 
              AND film_length <= 180 THEN 'Between 2 - 3 hours' 
         ELSE 'More then 3 hours long ' 
...
SELECT DISTINCT ( filmlen_groups ), 
                Count(title) 
                  OVER ( 
                    partition BY filmlen_groups) AS filmcount_bylength 
FROM   (SELECT title, 
               length, 
               CASE 
